...
def read_prc_csv(tic):
    ...
    # <COMPLETE THIS PART>
    tic = tic.lower()
    file_name = tic + '_prc.csv'
    url = os.path.join(cfg.DATADIR, file_name)
    df = pd.read_csv(url)
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for tic in tickers:
        ...
        df = cfg.standardise_colnames(df)  # Standardize column names
        df.rename(columns={prc_col: tic.lower()}, inplace=True)
        df = df[[tic.lower()]]
        tickers_df.append(df)

    combine_df = pd.concat(tickers_df, axis=1, join='outer')
    return combine_df
...
def mk_ret_df(prc_df):
    ...
    """
    # <COMPLETE THIS PART>
    ff_data = pd.read_csv(cfg.FF_CSV, index_col='Date', parse_dates=True)
    mkt_returns = ff_data['mkt']
    prc_df = prc_df.sort_index()
    stock_returns = prc_df.pct_change(fill_method=None)
    ret_df = pd.concat([stock_returns, mkt_returns], axis=1, join='inner')
...
def get_avg(df, col, year):
    ...
    """
    # <COMPLETE THIS PART>
    new_df = df.loc[df.index.year == year, col]
...
def get_ew_rets(df, tickers):
    ...
    """
    # <COMPLETE THIS PART>
    # Sort the DataFrame 'df' by its index (dates)
    df.sort_index(inplace=True)
    # Select all stocks in the 'portfolio'
    df_portfolio = df.loc[:, tickers]
    # Remove rows where all elements are missing values
    df_portfolio.dropna(axis=0, how='all', inplace=True)
    # Calculate the mean value of all stocks in the 'portfolio'
    portfolio_mean = df_portfolio.mean(axis=1, skipna=True)
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    # Sort series by its index (dates)
    ser.sort_index(inplace=True)
    # Select specified data from series
    ser = ser.loc[start:end]
    # Remove rows that contain only missing values
    ser = ser.dropna()
    # Transform returns in series to 1 + return format
    ser = ser.add(1)
    # Count the non-missing data points in the specified time period
    N = ser.count()
    # Compute the product of all data points in series
    tot_ret = ser.prod()
